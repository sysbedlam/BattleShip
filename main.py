...
#Тут не знал как правильно сделать. Можно было создать класс который запускает игру. Либо это должно быть
# статическим методом в Gamelogic. Либо функцией run. В общем оставил так.
game = Gamelogic()
game.create_ships()
while True:
    game.show_pole()
